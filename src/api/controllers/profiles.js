...
export default class ProfilesController {
  ...
  static async updateProfile(req, res) {
    ...
    if (!files && Object.keys(body) < 1)
      return res.status(400).send({ message: "Cannot update empty object" });

    if (files && Object.keys(body) < 1) {
      uploadImageWorker(files, params.id, "user", null);
      return res
        .status(200)
        .json({ status: 200, message: "Your image will be updated shortly" });
    }
    ...
    try {
      const updatedUser = await User.update(updatedProfile, {
        where: { id: params.id },
      });
  ...
  static async deleteProfile(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ status: 200, message: `User ${params.id} deleted.` });
    } catch (error) {
  ...
  static async getAllProfile(req, res) {
    const fetchedProfile = await User.findAll({
      attributes: ["username", "firstName", "lastName", "bio", "image"],
    });
    if (!fetchedProfile[0])
      return res
        .status(200)
        .send({ message: "No Users Profiles found!", data: fetchedProfile });
  ...
  static async follow(req, res) {
    ...
    if (registeredUser.id === req.user.id) {
      return res
        .status(400)
        .json({ errors: "You can not following yourself " });
    }
  ...
  static async unfollow(req, res) {
    ...
    return unfollowedUser
      ...
      : res.status(400).json({ error: `you do not follow ${username}` });
  }
    ...
    follows
      ...
      .then((data) => {
        if (!data[0]) {
          return res
            .status(200)
            .json({ message: "you do not have any followers currently", data });
        }
    ...
    follows
      ...
      .then((data) => {
        if (!data[0]) {
          return res
            .status(200)
            .json({ message: "you do not following anyone currently", data });
        }
